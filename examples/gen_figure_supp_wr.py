...
dataset = Path('2019-02-29_healthy_brain_network')
...
opt_train['eeg_setting'] = 'occipital_alpha_car'
...
# todo: set this range back to starting at 0
for ix_config in range(1, 2):
    ix_config_local = ix_config
    if ix_config == 0:
        fig_type = 'scan_fir_length'
        fig_type = 'scan_fir_length_longer'
        ix_config_local = 1
    elif ix_config == 1:
        fig_type = 'main_set_reduced_alt'
    else:
        fig_type = 'main_set_reduced_alt'
    ...
    # %% within run
    sub_result = paradigm.main_multiple_subject(datadir, dataset, sub_list, arch, opt_train, opt_test, max_n_run=1)
    mace = py_eegepe.summary.phase_plot.get_mace_run_averaged(sub_result)

    if do_plot:
        mace_test = py_eegepe.summary.phase_plot.get_mace_run_averaged(sub_result)
        ...
        # %%
        if fig_type == 'scan_fir_length' or fig_type == 'scan_fir_length_longer':
